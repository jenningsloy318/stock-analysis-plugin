...
- `--days N` → hot sector discovery focus window: 1=today (default), 5=this week, 10=recent 2 weeks, 20=this month. Controls which timeframe is weighted most heavily for "hot" ranking. Used in screen/pipeline/walk modes.
...
- `--min-headroom N` → minimum Growth Headroom score 1-10 (default: 5). Stocks scoring below this are filtered out at Stage 4 even if they pass price filter. Measures upside potential via TAM runway + growth gap + inflection signals + phase + valuation + money flow.
...
<stages>
  ...
  <stage n="4" name="Company Screening" agent="company-screener" modes="pipeline,screen">Screen companies across ALL top N sub-industries. Apply filters (market cap, growth, FCF, ROIC, price per --top-price, Growth Headroom per --min-headroom, universe filter). Dual-channel FCF filter: conservative (positive FCF required) or aggressive (negative FCF allowed for hyper-growth with 2yr cash runway). Cyclical adjustment (Step 3.5): auto-detect cycle position via margin_ratio; at TROUGH relax growth filter and use normalized P/E, at PEAK apply valuation penalty. Score on growth/profitability/moat/valuation/management/risk/liquidity. Select top M by score across ALL sub-industries — NOT quota per sub-industry.</stage>
  <stage n="4.5" name="Screening Validation" agent="report-validator" modes="pipeline,screen">Validate screening outputs: sub-industry leaderboard has 10+ entries with valid GICS codes, company watchlist has 10+ companies, price filter applied, headroom filter applied, universe filter applied, 推荐标的排名 format correct. Blocks report generation if screening is incomplete.</stage>
  ...
  <stage n="walk" name="Bottleneck Walk" agent="roadmap-walker" modes="walk">Top-down chain decomposition: anchor quantitative dated demand roadmap → reverse-walk chain finished-product→raw-substrate (≥5 layers) → score 4-element chokepoint checklist per layer → identify candidates in chokepoint layers (score ≥3) → run score_bottleneck_asymmetry.py for each → write walk_roadmap.json, walk_chain.json, walk_candidates.json, walk.md. Replaces Stages 2-4 in walk mode (roadmap-walker replaces company screening with supply-chain-based candidate selection). After walk, select TOP 3-5 candidates by asymmetry_composite score for full deep-dive (Stages 5-15). Universal across industries (AI infra, EV, robotics, defense, solar, biopharma, grid, semi capex, materials). Reference: references/frameworks_bottleneck_investing.md.</stage>

  <stage n="16" name="Scoring & Cross-Check" agent="scorer" modes="pipeline,analyze,compare,walk">Deterministic scoring (compute_scores.py) for each company. Cross-check contradictions (cross_check.py). Bayesian conviction calibration (calibrate_conviction.py). LLM agents may adjust Moat and Management ±2.0 based on qualitative findings. Rank companies by composite score.</stage>
  <stage n="16.5" name="Score Validation" agent="report-validator" modes="pipeline,analyze,compare,walk">Validate Stage 16 scoring: all 11 components present in 1-10 range, composite matches weighted sum, rating bracket consistent, no unresolved contradictions, ranking sorted correctly. Blocks report generation if scoring is invalid.</stage>
  ...
  <stage n="16.7" name="Judge Panel" agent="quant-analyst" modes="pipeline,analyze,compare">For top 5 picks: conflict-based multi-framework assessment. 4 investment-framework lenses (Buffett / Lynch / Marks / Druckenmiller), each independently rates 0-10 with verdict (BUY/HOLD/AVOID) AND explicit disagreement points with other frameworks. Key output is NOT just scores — it is the identification of TOP 1-2 framework disagreements per company, framed as "这是真正的投资决策点" with observable resolution metrics (e.g., "gross margin next 2Q resolves Buffett-vs-Marks conflict"). Each disagreement maps to conditional outcomes (metric positive → conclusion A, metric negative → conclusion B). Position type (core/satellite/option) derived from consensus strength × disagreement severity. Persisted to `judge_panel.json`.</stage>
  ...
  <stage n="18" name="Best Picks Highlight" agent="equity-report-writer">After ALL reports pass validation, write HIGHLIGHTS_BEST_PICKS.md to ./reports/[RUN_ID]/. Groups picks by position type (core/satellite/option from judge_panel.json) instead of pure ranking. Each group: 核心仓位推荐（高确定性）, 成长卫星推荐（中确定性）, 期权投机推荐（高弹性）. Per-company: rank, ticker, company name, current price, composite score, conviction, 2-sentence thesis, kill switch, key catalyst, 框架共识, 对手方验证 status. Ends with 组合互补性检查 (industry concentration + style homogeneity). ⚠️ caution notes for flagged picks.</stage>
...
<rules>
  ...
  <rule name="Price Filter" mandatory="true">Price filter (`--top-price`, default 200) applies ONLY during Stage 4 (Company Screening). ALL markets: US stocks < $N, China A-shares < ¥N, all other markets < equivalent of $N USD. Set 0 to disable. After screening, do NOT re-filter or exclude companies during analysis (5-15) or report generation (17-18). Exception: if user specifies a specific ticker (analyze/compare mode), proceed regardless of price (user override).</rule>
  <rule name="Headroom Filter" mandatory="true">Growth Headroom filter (`--min-headroom`, default 5) applies during Stage 4. Run `compute_growth_headroom.py` on all price-passing candidates. Reject stocks scoring below threshold — eliminates "fully developed" low-upside stocks (high TAM penetration, decelerating growth, expensive valuation, institutional distribution). After screening, do NOT re-filter.</rule>
  ...
  <rule name="Run Directory">Each run creates `./reports/[RUN_ID]/` where RUN_ID = YYYYMMDDHHmm in LOCAL TIME.</rule>
...
<criteria name="Skip Conditions">
  ...
  Stage 16-16.7 (Scoring + Verify + Panel): SKIP for screen modes. RUN for walk mode (on deep-dived candidates).
  ...
  Stage 17.4 (Completeness Critic): SKIP for screen modes. RUN for walk mode (on deep-dived candidates).
