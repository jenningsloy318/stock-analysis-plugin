...
<workflow>
  ...
  <stage n="16.5" name="Score Validation" agent="report-validator" modes="pipeline,analyze,compare,walk" runs-in="workflow">All 11 components in range, composite consistent, ranking sorted.</stage>
...
<modes>
  ...
  <mode name="analyze">
    ...
    <stages>0→1→1.5→5-15(waves)→16→16.5→16.6→16.7→17→17.4→17.5→18→18.5→19(best picks + validation + cleanup)</stages>
  </mode>
  ...
  <mode name="compare">
    ...
    <stages>0→1→1.5→5-15(waves)→16(rank+merge)→16.5→16.6→16.7→17→17.4→17.5→18→18.5→19(comparison + validation + best picks + cleanup)</stages>
