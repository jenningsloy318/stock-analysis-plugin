...
def compute_l1_technical(trade_signals_data: dict | None) -> dict:
    ...
    try:
        ...
        if total_weight == 0:
            ...
            net_dir = DIR_BUY if net_score > 0.1 else (DIR_SELL if net_score < -0.1 else DIR_NEUTRAL)
...
def compute_l2_factor(
    ...
    try:
        ...
        val_score = _safe_get(scores_data, "components", "valuation_attractiveness", "score", default=None)
        ...
        if val_score is not None:
            if val_score >= 7:
                signals.append(_make_signal(
                    "F2", "价值因子", DIR_BUY, 0.70 + (val_score - 7) * 0.05,
                    f"估值吸引力评分{val_score:.1f}/10, 显著低估",
                    {"valuation_score": val_score},
                ))
            elif val_score <= 3:
                signals.append(_make_signal(
                    "F2", "价值因子", DIR_SELL, 0.65,
        ...
        fh_score = _safe_get(scores_data, "components", "financial_health", "score", default=None)
        if fh_score is None:
        ...
        revision_trend = _safe_get(factors_data, "earnings_revision", "trend", default=None)
        revision_score = _safe_get(factors_data, "earnings_revision", "score", default=None)
        if revision_score is None:
            revision_score = _safe_get(scores_data, "components", "earnings_revision", "score", default=None)

        if revision_score is not None and revision_score >= 7:
            is_upgrade = revision_trend in ("strong_upgrade", "upgrade", "positive")
            if is_upgrade:
                signals.append(_make_signal(
                    "F4", "盈利修正因子", DIR_BUY, 0.75,
                    f"盈利预期上调趋势, 修正评分{revision_score:.1f}",
                    {"revision_trend": revision_trend, "revision_score": revision_score},
                ))
        elif revision_score is not None and revision_score <= 3:
            signals.append(_make_signal(
                "F4", "盈利修正因子", DIR_SELL, 0.60,
        ...
        if total == 0:
            ...
            net_dir = DIR_BUY if net_score > 0.15 else (DIR_SELL if net_score < -0.15 else DIR_NEUTRAL)
            layer_conf = abs(net_score)
...
def compute_l3_event(
    ...
    if not sentiment_data and not earnings_edge_data and not activist_data and not capital_structure_data:
        return {
            "layer_id": layer_id,
            "layer_name": layer_name,
            "available": False,
    ...
    try:
        ...
        beat_pct = _safe_get(earnings_edge_data, "latest_surprise", "beat_pct", default=None)
        if beat_pct is None:
            beat_pct = _safe_get(sentiment_data, "earnings", "surprise_pct", default=None)
        ...
        insider_net = _safe_get(sentiment_data, "insider", "net_buy_value_90d", default=None)
        if insider_net is None:
            insider_net = _safe_get(sentiment_data, "insider_trading", "net_purchase_value", default=None)

        if insider_net is not None:
            if insider_net > 500_000:
                signals.append(_make_signal(
                    "E2", "内部人买入", DIR_BUY, 0.75,
                    f"90天内部人净买入${insider_net/1e6:.2f}M > $500K, 管理层信心强",
                    {"net_insider_buy_90d": insider_net},
                ))
            elif insider_net < -1_000_000:
                signals.append(_make_signal(
                    "E2", "内部人买入", DIR_SELL, 0.55,
        ...
        buyback_pct = _safe_get(capital_structure_data, "buyback", "pct_of_float", default=None)
        if buyback_pct is None:
            buyback_pct = _safe_get(capital_structure_data, "buyback_roi", "buyback_yield", default=None)
        buyback_active = _safe_get(capital_structure_data, "buyback", "active", default=False)

        if buyback_pct is not None and buyback_pct > 0.05:
            signals.append(_make_signal(
                "E3", "回购信号", DIR_BUY, 0.65,
                f"活跃回购占流通股{buyback_pct:.1%} > 5%, 股价下限支撑",
                {"buyback_pct_float": buyback_pct, "buyback_active": buyback_active},
            ))
        elif buyback_pct is not None and buyback_pct > 0.03 and buyback_active:
            signals.append(_make_signal(
                "E3", "回购信号", DIR_BUY, 0.45,
        ...
        if has_13d:
            ...
            signals.append(_make_signal(
                "E4", "激进投资者进入", DIR_BUY, conf,
                f"13D备案: {activist_name or '未知'} 持股{activist_stake:.1%}" if activist_stake else f"13D备案: {activist_name or '未知激进投资者'}",
                {"has_13d": True, "activist_name": activist_name, "stake_pct": activist_stake},
            ))
...
def compute_l4_institutional(
    ...
    try:
        ...
        ctb_rising = _safe_get(short_interest_data, "ctb_trend", default=None) in ("rising", "up")
        ...
        if si_pct is not None and si_pct > 0.20 and dtc is not None and dtc > 5:
            ...
            signals.append(_make_signal(
                "M1", "做空挤压设置", DIR_BUY, conf,
                f"空头比例{si_pct:.1%} > 20%, 覆盖天数{dtc:.1f} > 5, CTB{'上升' if ctb_rising else '稳定'}, 逼空风险",
                {"si_pct": si_pct, "days_to_cover": dtc, "ctb_rising": ctb_rising},
            ))
        elif si_pct is not None and si_pct > 0.15 and squeeze_score is not None and squeeze_score > 7:
            signals.append(_make_signal(
                "M1", "做空挤压设置", DIR_BUY, 0.55,
        ...
        obv_divergence = _safe_get(money_flow_data, "obv", "price_divergence", default=None)
        ...
        if obv_trend in ("rising", "up", "strongly_rising"):
            ...
            if price_flat or obv_divergence in ("positive", "bullish"):
                signals.append(_make_signal(
                    "M2", "隐性吸筹", DIR_BUY, 0.65,
                    f"OBV趋势{obv_trend}, 价格{price_trend or '横盘'}, 隐性资金流入",
                    {"obv_trend": obv_trend, "price_trend": price_trend, "divergence": obv_divergence},
                ))
        elif obv_trend in ("falling", "down", "strongly_falling") and price_trend in ("rising", "up"):
            signals.append(_make_signal(
                "M2", "隐性吸筹", DIR_SELL, 0.60,
        ...
        inflow_days = _safe_get(money_flow_data, "consecutive_inflow_days", default=None)
        if inflow_days is None:
            inflow_days = _safe_get(money_flow_data, "net_flow", "consecutive_inflow", default=None)
        vol_price_sym = _safe_get(money_flow_data, "volume_price_symmetry", default=None)

        if inflow_days is not None and inflow_days >= 5:
            conf = 0.70 if vol_price_sym in ("symmetric", "positive", "confirming") else 0.55
            signals.append(_make_signal(
                "M3", "机构持续流入", DIR_BUY, conf,
                f"连续{inflow_days}天净流入, 量价{'对称' if vol_price_sym in ('symmetric', 'positive', 'confirming') else '待确认'}",
                {"consecutive_inflow_days": inflow_days, "vol_price_symmetry": vol_price_sym},
            ))
        elif inflow_days is not None and inflow_days <= -5:
            signals.append(_make_signal(
                "M3", "机构持续流入", DIR_SELL, 0.60,
        ...
        sector_flow = _safe_get(money_flow_data, "sector_etf_flow", "trend", default=None)
...
def compute_l5_options(options_data: dict | None) -> dict:
    ...
    try:
        ...
        # call_vol > 3x avg → BUY
        call_vol_ratio = _safe_get(options_data, "unusual_activity", "call_volume_ratio", default=None)
        if call_vol_ratio is None:
            call_vol_ratio = _safe_get(options_data, "call_vol_vs_avg", default=None)
        put_vol_ratio = _safe_get(options_data, "unusual_activity", "put_volume_ratio", default=None)

        if call_vol_ratio is not None and call_vol_ratio > 3.0:
            signals.append(_make_signal(
                "O1", "异常期权活动", DIR_BUY, 0.70,
        ...
        if pcr is not None:
            if pcr > 1.5:
                signals.append(_make_signal(
                    "O2", "看跌看涨比极端", DIR_BUY, 0.60,
                    f"PCR={pcr:.2f} > 1.5, 极端恐慌, 反向指标看多",
                    {"put_call_ratio": pcr},
                ))
            elif pcr < 0.5:
                signals.append(_make_signal(
                    "O2", "看跌看涨比极端", DIR_SELL, 0.55,
        ...
        days_to_expiry = _safe_get(options_data, "max_pain", "days_to_expiry", default=None)

        if max_pain is not None and current_price is not None:
            gap_pct = (max_pain - current_price) / current_price if current_price > 0 else 0
            near_expiry = days_to_expiry is not None and days_to_expiry <= 7

            if gap_pct > 0.03 and near_expiry:
                signals.append(_make_signal(
                    "O4", "最大痛点磁性", DIR_BUY, 0.50,
                    f"当前价低于最大痛点{gap_pct:.1%}, 距到期{days_to_expiry}天, 短期上拉压力",
                    {"max_pain": max_pain, "current_price": current_price, "gap_pct": gap_pct, "dte": days_to_expiry},
                ))
            elif gap_pct < -0.03 and near_expiry:
                signals.append(_make_signal(
                    "O4", "最大痛点磁性", DIR_SELL, 0.45,
...
def compute_l6_alt_data(
    ...
    try:
        ...
        app_growth = _safe_get(alternatives_data, "app_store", "download_growth_mom", default=None)
        if app_growth is None:
            app_growth = _safe_get(alternatives_data, "digital_footprint", "app_growth", default=None)
        web_traffic_growth = _safe_get(alternatives_data, "web_traffic", "growth_mom", default=None)

        if app_growth is not None and app_growth > 0.20:
            signals.append(_make_signal(
                "A1", "应用数字动量", DIR_BUY, 0.65,
        ...
        # R&D jobs up 30%+ → BUY
        rd_hiring_growth = _safe_get(alternatives_data, "hiring", "rd_growth", default=None)
        if rd_hiring_growth is None:
            rd_hiring_growth = _safe_get(alternatives_data, "glassdoor", "rd_job_growth", default=None)
        total_hiring = _safe_get(alternatives_data, "hiring", "total_growth", default=None)

        if rd_hiring_growth is not None and rd_hiring_growth > 0.30:
            signals.append(_make_signal(
                "A2", "招聘信号", DIR_BUY, 0.55,
        ...
        social_sentiment = _safe_get(alternatives_data, "social", "sentiment_score", default=None)
        if social_sentiment is None:
            social_sentiment = _safe_get(news_nlp_data, "social_sentiment", "score", default=None)
        sentiment_extreme = _safe_get(news_nlp_data, "sentiment_extreme", default=None)

        if social_sentiment is not None:
            if social_sentiment < -0.7:
                signals.append(_make_signal(
                    "A3", "社交情绪极端", DIR_BUY, 0.55,
                    f"社交情绪={social_sentiment:.2f}, 极端负面, 反向指标看多",
                    {"social_sentiment": social_sentiment},
                ))
            elif social_sentiment > 0.8:
                signals.append(_make_signal(
                    "A3", "社交情绪极端", DIR_SELL, 0.45,
        ...
        search_spike = _safe_get(alternatives_data, "google_trends", "brand_spike_ratio", default=None)
        if search_spike is None:
            search_spike = _safe_get(alternatives_data, "search", "spike_ratio", default=None)

        if search_spike is not None and search_spike > 2.0:
            signals.append(_make_signal(
                "A4", "搜索趋势", DIR_BUY, 0.50,
...
def compute_l7_cross_asset(
    ...
    try:
        ...
        if vix is not None:
            if vix > 35 and vix_term in ("contango", "returning_contango", "normalizing"):
                signals.append(_make_signal(
                    "X2", "VIX体制", DIR_BUY, 0.65,
                ...
                signals.append(_make_signal(
                    "X2", "VIX体制", DIR_NEUTRAL, 0.40,
                    f"VIX={vix:.1f} > 35 + 倒挂, 恐慌持续, 等待信号",
                    {"vix": vix, "vix_term_structure": vix_term},
                ))
            elif vix < 12:
                signals.append(_make_signal(
                    "X2", "VIX体制", DIR_SELL, 0.45,
        ...
        if cu_au_trend is None:
            cu_au_trend = _safe_get(breadth_data, "macro_signals", "copper_gold_ratio_trend", default=None)
        ...
        if cu_au_trend in ("rising", "up", "improving"):
            signals.append(_make_signal(
                ...
                {"copper_gold_trend": cu_au_trend, "copper_gold_ratio": cu_au_ratio},
            ))
        elif cu_au_trend in ("falling", "down", "deteriorating"):
            signals.append(_make_signal(
                "X4", "铜金比", DIR_SELL, 0.50,
...
def _compute_cross_layer_confirmation(layers: list[dict]) -> dict:
    """Count how many layers agree on the same direction.

    Returns confirmation score:
    ...
    dominant_direction = DIR_BUY if buy_count > sell_count else (DIR_SELL if sell_count > buy_count else DIR_NEUTRAL)
...
def _determine_verdict(weighted_dir: float, confirm: dict) -> dict:
    """Determine final verdict based on weighted direction and confirmation.

    Verdict rules:
        STRONG_BUY:  weighted_dir > 0.5  AND confirmation >= 6
        BUY:         weighted_dir > 0.3  AND confirmation >= 4
        LEAN_BUY:    weighted_dir > 0.1
        NEUTRAL:     -0.1 <= weighted_dir <= 0.1
        LEAN_SELL:   weighted_dir < -0.1
        SELL:        weighted_dir < -0.3 AND confirmation >= 4
        STRONG_SELL: weighted_dir < -0.5 AND confirmation >= 6
    """
    conf_score = confirm["confirmation_score"]

    if weighted_dir > 0.5 and conf_score >= 6:
        verdict = "STRONG_BUY"
        verdict_cn = "强烈买入"
        action = "建仓/加仓"
    elif weighted_dir > 0.3 and conf_score >= 4:
        verdict = "BUY"
        verdict_cn = "买入"
        action = "建仓"
        ...
        verdict_cn = "偏空"
        action = "减仓/观望"
    elif weighted_dir > -0.5 or conf_score < 6:
        verdict = "SELL"
        verdict_cn = "卖出"
        action = "减仓/清仓"
    else:
...
def _detect_confluence(layers: list[dict]) -> list[dict]:
    ...
    tech_buy = any(sid.startswith("B") for sid in signal_ids)
    flow_buy = any(sid in ("M2", "M3") and all_signals[sid]["direction"] > 0 for sid in signal_ids if sid in all_signals)
    options_buy = any(sid in ("O1", "O2") and all_signals[sid]["direction"] > 0 for sid in signal_ids if sid in all_signals)
    ...
    if has_x1_sell and (has_x2_sell or has_x5_sell):
        confluences.append({
...
def _identify_risk_factors(layers: list[dict], verdict: dict) -> list[dict]:
    ...
    avg_conf = sum(l.get("layer_confidence", 0) for l in available) / max(len(available), 1)
    if avg_conf < 0.4:
        risks.append({
            "risk_id": "R2",
            "name_cn": "低置信度",
    ...
    missing = [lid for lid, w in LAYER_WEIGHTS.items()
               if w >= 0.15 and not any(l["layer_id"] == lid and l.get("available") for l in layers)]
    if missing:
        risks.append({
            "risk_id": "R3",
            "name_cn": "关键数据缺失",
    ...
    if verdict["verdict"] in ("LEAN_BUY", "BUY"):
        strong_buy_layers = [l for l in buy_layers if l["layer_id"] not in ("L6_alt_data",)]
...
def aggregate_signals(
    ...
    buy_signals = sum(
        ...
        for l in layers if l.get("available")
    )
    sell_signals = sum(
        sum(1 for s in l.get("signals", []) if s["direction"] < 0)
        for l in layers if l.get("available")
...
def main():
    ...
    parser.add_argument("--trade-signals-json", help="Path to compute_trade_signals.py output JSON (L1)")
    parser.add_argument("--factors-json", help="Path to compute_factors.py output JSON (L2)")
    parser.add_argument("--sentiment-json", help="Path to fetch_sentiment.py output JSON (L3)")
    parser.add_argument("--earnings-edge-json", help="Path to compute_earnings_edge.py output JSON (L3)")
    parser.add_argument("--options-json", help="Path to calculate_options.py output JSON (L5)")
    parser.add_argument("--alternatives-json", help="Path to fetch_alternatives.py output JSON (L6)")
    parser.add_argument("--news-nlp-json", help="Path to fetch_news_nlp.py output JSON (L6)")
    parser.add_argument("--breadth-json", help="Path to fetch_market_breadth.py output JSON (L7)")
    parser.add_argument("--credit-json", help="Path to fetch_credit.py output JSON (L7)")
    parser.add_argument("--short-interest-json", help="Path to fetch_short_interest.py output JSON (L4)")
    parser.add_argument("--activist-json", help="Path to fetch_activist_exposure.py output JSON (L3)")
    parser.add_argument("--capital-structure-json", help="Path to fetch_capital_structure.py output JSON (L3)")
    parser.add_argument("--tech-json", help="Path to fetch_technicals.py output JSON (L2)")
    parser.add_argument("--money-flow-json", help="Path to money flow data JSON (L4)")
    parser.add_argument("--scores-json", help="Path to compute_scores.py output JSON (L2)")
    ...
    # L3: Event-Driven
    layers.append(compute_l3_event(sentiment_data, earnings_edge_data, activist_data, capital_structure_data))
