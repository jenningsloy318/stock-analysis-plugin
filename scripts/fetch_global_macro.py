...
def fetch_world_bank(indicator: str, country: str, date_range: str = "2018:2026") -> dict | None:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if len(data) >= 2 and data[1]:
                ...
                if values:
                    return {
                        "indicator_name": data[1][0].get("indicator", {}).get("value", indicator),
                        "country": country,
...
def fetch_region_data(region: str) -> dict:
    """Fetch all available macro indicators for a region."""
    result = {
        "region": region,
    ...
    region_rates = {}
    mapping = {"EU": "ecb_main_refinancing_rate", "CN": "pboc_1y_mlf",
               "JP": "boj_policy_rate", "UK": "boe_bank_rate"}
...
def main():
    ...
    parser.add_argument("--regions", default="EU,CN,JP,UK",
                        help="Comma-separated region codes: EU,CN,JP,UK,KR,IN,BR,CA,AU,CH")
    parser.add_argument("--all", action="store_true",
                        help="Fetch all available regions")
    ...
    if args.all:
        ...
        regions = [r.strip().upper() for r in args.regions.split(",") if r.strip().upper() in REGION_COUNTRIES]
    ...
    for region in regions:
        result["data"][region] = fetch_region_data(region)
