...
<process>
  <step n="0" name="Setup">Determine screening scope (all sectors / specific sector / theme), fetch macro context via fetch_macro.py, compute BOTH sector RS AND sub-industry RS (`--level sub-industry`), create output directory, initialize state via `persist.py init SCREEN-[TIMESTAMP] --report-type screen`, load `references/gics_taxonomy.md` and `references/data_source_matrix.md`, and write `./reports/screening/source-plan.md`. All 3 horizons (long/mid/short) are produced automatically — do NOT ask user.</step>
