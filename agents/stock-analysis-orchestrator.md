...
<step n="4" name="Spawn Macro">Update [TICKER]-tracking.json: stages 2-3 → "completed", stages 4-5 → "in_progress". Spawn macro-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stages 4-5 (Macro + Geopolitics). Output to ./reports/[RUN_ID]/.</step>
<step n="5" name="Spawn Quant">Update [TICKER]-tracking.json: stages 4-5 → "completed", stages 6-7 → "in_progress". Spawn quant-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stages 6-7 (Valuation + Market Regime). Output to ./reports/[RUN_ID]/.</step>
<step n="6" name="Spawn Risk">Update [TICKER]-tracking.json: stages 6-7 → "completed", stage 8 → "in_progress". Spawn risk-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stage 8 (Risk Assessment & Synthesis). Output to ./reports/[RUN_ID]/.</step>
<step n="7" name="Spawn Alt Data">Update [TICKER]-tracking.json: stage 8 → "completed", stage 9 → "in_progress". Spawn alt-data-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stage 9 (Alternative Data). Output to ./reports/[RUN_ID]/.</step>
<step n="8" name="Run Deterministic Scoring">Update [TICKER]-tracking.json: stage 9 → "completed", stage 10 → "in_progress". Run compute_scores.py against all script outputs to produce reproducible component scores. LLM agents may adjust Moat and Management scores ±2.0 based on qualitative findings.</step>
...
<constraint>Tracking JSON (./reports/[RUN_ID]/[TICKER]-tracking.json) MUST be updated BEFORE advancing to the next stage — mark current stage "completed" with timestamp, then set next stage "in_progress" with timestamp</constraint>
