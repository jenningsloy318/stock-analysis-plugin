...
Where RUN_ID = YYYYMMDDHHmm (e.g., 202605281430), set once at run start. NNN is the rank index (001 for single stock).

## 3. Workflow

<step n="0" name="Team Creation">Identify ticker from user request. Set RUN_ID via `date +%Y%m%d%H%M` (e.g., 202605251430). Create output directory: `./reports/[RUN_ID]/001-[TICKER]/` (single stock: rank is always 001; multi-stock: create temp dirs first, rename with rank prefix after Stage 10 scoring). Create tracking file `./reports/[RUN_ID]/001-[TICKER]/tracking.json` with all stages initialized as "pending", stage 0 set to "in_progress". Create agent team IMMEDIATELY — this is the FIRST action before any scripts or data fetches. Claude Code: TeamCreate({ name: "stock-analysis-[TICKER]-[RUN_ID]" }). Gemini CLI: team is implicit. Always produce all 3 report types (long/mid/short) — do NOT ask user for horizon.</step>
<step n="1" name="Spawn Data Fetch">Update tracking.json: stage 0 → "completed", stage 1 → "in_progress". Spawn search-agent (team_name: "stock-analysis-[TICKER]-[RUN_ID]") to perform all triage data collection: ensure output directory ./reports/[RUN_ID]/ exists, run fetch_financials.py, fetch_macro.py, fetch_global_macro.py, fetch_economic_surprises.py, fetch_credit.py, forecast.py, calculate_metrics.py, diff_filings.py, fetch_market_breadth.py --skip-constituents, fetch_theme_performance.py, persist.py init. Market breadth/theme data provides Stage 7 (Market Regime) context for the quant-analyst. Agent writes results to ./reports/[RUN_ID]/. Terminate after completion.</step>
<step n="2" name="Spawn Fundamentals">Update tracking.json: stage 1 → "completed", stages 2-3 → "in_progress". Spawn fundamental-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stages 1-2 (Company Fundamentals + Executive/Board). Output to ./reports/[RUN_ID]/.</step>
...
<step n="4" name="Spawn Macro">Update tracking.json: stages 2-3 → "completed", stages 4-5 → "in_progress". Spawn macro-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stages 4-5 (Macro + Geopolitics). Output to ./reports/[RUN_ID]/.</step>
<step n="5" name="Spawn Quant">Update tracking.json: stages 4-5 → "completed", stages 6-7 → "in_progress". Spawn quant-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stages 6-7 (Valuation + Market Regime). Output to ./reports/[RUN_ID]/.</step>
<step n="6" name="Spawn Risk">Update tracking.json: stages 6-7 → "completed", stage 8 → "in_progress". Spawn risk-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stage 8 (Risk Assessment & Synthesis). Output to ./reports/[RUN_ID]/.</step>
<step n="7" name="Spawn Alt Data">Update tracking.json: stage 8 → "completed", stage 9 → "in_progress". Spawn alt-data-analyst (team_name: "stock-analysis-[TICKER]-[RUN_ID]") for Stage 9 (Alternative Data). Output to ./reports/[RUN_ID]/.</step>
<step n="8" name="Run Deterministic Scoring">Update tracking.json: stage 9 → "completed", stage 10 → "in_progress". Run compute_scores.py against all script outputs to produce reproducible component scores. LLM agents may adjust Moat and Management scores ±2.0 based on qualitative findings.</step>
...
<step n="10" name="Spawn Report Writer">Update tracking.json: stage 10 → "completed", stage 11 → "in_progress". Pre-compute final report filenames using rank-prefixed naming: `./reports/[RUN_ID]/001-[TICKER]/001-[TICKER]_long_[YYYY-MM-DD].md`, `./reports/[RUN_ID]/001-[TICKER]/001-[TICKER]_mid_[YYYY-MM-DD].md`, `./reports/[RUN_ID]/001-[TICKER]/001-[TICKER]_short_[YYYY-MM-DD].md` (single stock: 001; multi-stock: use assigned rank NNN). Pass these EXACT filenames to the report writer in the spawn prompt. Spawn equity-report-writer (team_name: "stock-analysis-[TICKER]-[RUN_ID]") with: stage summaries, scoring output, the 3 target filenames, and explicit instruction "ALL reports MUST be written in Chinese (中文)". Writer produces ALL 3 reports from the same data.</step>
<step n="11" name="Quality Gate & Cleanup">Update tracking.json: stage 11 → "completed". Run pre-delivery checklist, validate fact integrity, run validate_report.py to verify quality gates, deliver final 3 reports to user. Then cleanup: (1) delete all intermediate files (./reports/[RUN_ID]/NNN-[TICKER]/stage*.md, raw-data.json, etc.), (2) terminate all agents, (3) delete team: TeamDelete({ name: "stock-analysis-[TICKER]-[RUN_ID]" }). Keep tracking.json + 3 final report files in ./reports/[RUN_ID]/NNN-[TICKER]/. Post-delivery: run event_study.py, calibrate_conviction.py, portfolio_context.py, backtest.py.</step>
...
<constraint>Tracking JSON (./reports/[RUN_ID]/NNN-[TICKER]/tracking.json) MUST be updated BEFORE advancing to the next stage — mark current stage "completed" with timestamp, then set next stage "in_progress" with timestamp</constraint>
